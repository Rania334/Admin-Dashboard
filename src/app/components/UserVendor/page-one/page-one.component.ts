...
export class PageOneComponent {
  ...
  filterList() {
    ...
    if (this.filterType === 'users' || this.filterType === 'all') {
      this.filteredUsers = this.users.filter(user =>
        (user.name && user.name.toLowerCase().includes(search)) ||
        (user.email && user.email.toLowerCase().includes(search)) ||
        (user.homeAddress && user.homeAddress.toLowerCase().includes(search)) ||
        (user.mobileNumber && user.mobileNumber.toLowerCase().includes(search))
      );
    }
    if (this.filterType === 'vendors' || this.filterType === 'all') {
      this.filteredVendors = this.vendors.filter(vendor =>
        (vendor.name && vendor.name.toLowerCase().includes(search)) ||
        (vendor.email && vendor.email.toLowerCase().includes(search)) ||
        (vendor.storeName && vendor.storeName.toLowerCase().includes(search)) ||
        (vendor.storeAddress && vendor.storeAddress.toLowerCase().includes(search)) ||
        (vendor.mobileNumber && vendor.mobileNumber.toLowerCase().includes(search))
      );
    }
  }
